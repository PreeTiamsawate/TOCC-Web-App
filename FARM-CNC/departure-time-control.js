const timeControl = function() {
    ...
    for (let flightBox of flightBoxes) {
        ...
        const cargoCompTime = new Date(Date.parse($(flightBox).find(".inputs-from-backend .Cargo-COMP-Time_millisecond").text()))
        const boardingCompTime = new Date(Date.parse($(flightBox).find(".inputs-from-backend .Boarding-COMP-Time_millisecond").text()))
        const pushbackStbyTime = new Date(Date.parse($(flightBox).find(".inputs-from-backend .Pushback-STBY-Time_millisecond").text()))
        const boardingTime = new Date(Date.parse($(flightBox).find(".inputs-from-backend .Boarding-Time_millisecond").text()))
        const cateringCompTime = new Date(Date.parse($(flightBox).find(".inputs-from-backend .Catering-COMP-Time_millisecond").text()))
        const cleanerCompTime = new Date(Date.parse($(flightBox).find(".inputs-from-backend .Cleaner-COMP-Time_millisecond").text()))
        const rampBusTime = new Date(Date.parse($(flightBox).find(".inputs-from-backend .Ramp-Bus-Time_millisecond").text()))
        const paxStepTime = new Date(Date.parse($(flightBox).find(".inputs-from-backend .PAX-Step-Time_millisecond").text()))
        const cargoArrTime = new Date(Date.parse($(flightBox).find(".inputs-from-backend .Cargo-ARR-Time_millisecond").text()))
        const gateOpenTime = new Date(Date.parse($(flightBox).find(".inputs-from-backend .Gate-Open-Time_millisecond").text()))
        const cleanerArrTime = new Date(Date.parse($(flightBox).find(".inputs-from-backend .Cleaner-ARR-Time_millisecond").text()))
        const cateringArrTime = new Date(Date.parse($(flightBox).find(".inputs-from-backend .Catering-ARR-Time_millisecond").text()))
        const crewArrTime = new Date(Date.parse($(flightBox).find(".inputs-from-backend .Crew-ARR-Time_millisecond").text()))
        ...
        let aircraftStartTime = new Date(Date.parse($(flightBox).find(".inputs-from-backend .START_TIME_millisecond").text()));
        let aircraftArriveTime = new Date(Date.parse($(flightBox).find(".inputs-from-backend .ARRIVE_TIME_millisecond").text()));
        let repairEstCompTime = new Date(Date.parse($(flightBox).find(".inputs-from-backend .REPAIR_EST_COMPLETE_TIME_millisecond").text()))
        let aircraftPowerUpTime = new Date(Date.parse($(flightBox).find(".inputs-from-backend .AIRCRAFT_POWER_UP_TIME_millisecond").text()))
        let aircraftReleasedTime = new Date(Date.parse($(flightBox).find(".inputs-from-backend .AIRCRAFT_RELEASED_TIME_millisecond").text()))
